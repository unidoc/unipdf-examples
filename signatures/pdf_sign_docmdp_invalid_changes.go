/*
 * This example showcases how to sign a PDF document with the DocMDP restriction and add some invalid changes.
 * The file is signed using a private/public key pair.
 *
 * $ ./pdf_sign_docmdp_invalid_changes <INPUT_PDF_PATH> <OUTPUT_PDF_PATH>
 */
package main
...
import (
	...
	"github.com/unidoc/unipdf/v5/model/mdp"
...
func main() {
	args := os.Args
	if len(args) < 3 {
		log.Fatalln("Usage: go run pdf_sign_docmdp_invalid_changes INPUT_PDF_PATH OUTPUT_PDF_PATH")
	}
	...
	err = addSomeInvalidChanges(pdfReader2, outputPath)
	if err != nil {
		log.Fatalf("Fail: %v\n", err)
	}
...
func addSomeInvalidChanges(reader *model.PdfReader, outputPath string) error {
	// Create appender.
...
func addSignature(reader *model.PdfReader, pageNum int, outputPath string) ([]byte, error) {
	...
	handler, err := sighandler.NewDocMDPHandler(inner_handler, mdp.FillFormsAndAnnots)
	if err != nil {
		return nil, err
	}
