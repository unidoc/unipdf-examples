...
- [pdf_drop_caps.go](pdf_drop_caps.go) The example demonstrates drop caps functionality for `StyledParagraph`, including both traditional drop caps (DropCapsDrop) and inline drop caps (DropCapsInline) with various styling options.
...
- [pdf_gradient_text.go](pdf_gradient_text.go) The example demonstrates filling text with linear and radial color gradients, including scoping a gradient to a single chunk of a `StyledParagraph`.
- [pdf_highlight_text.go](pdf_highlight_text.go) The example demonstrates how to highlight target text inside a PDF file.
...
- [pdf_search_replace_advanced.go](pdf_search_replace_advanced.go) The example demonstrates a more advanced search and replace that takes font encoding into account and handles search terms split across multiple text operators.
